...
static void
action_snap_notes (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    g_autofree gchar *path = snap_actions_get_selected_path (user_data);
    if (path == NULL)
        return;
    ...
    GtkWidget *toplevel = GTK_WIDGET (gtk_widget_get_root (GTK_WIDGET (user_data)));
    snap_notes_dialog_present (path, GTK_WINDOW (toplevel));
}

static void
action_snap_compare (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    GList *paths = snap_actions_get_selected_paths (user_data);
    if (paths == NULL || paths->next == NULL)
    {
        g_list_free_full (paths, g_free);
        return;
    }

    const gchar *path_a = paths->data;
    const gchar *path_b = paths->next->data;

    GtkWidget *toplevel = GTK_WIDGET (gtk_widget_get_root (GTK_WIDGET (user_data)));
    snap_compare_dialog_present (path_a, path_b, GTK_WINDOW (toplevel));
    g_list_free_full (paths, g_free);
}
...
static const GActionEntry snap_view_entries[] =
{
    ...
    { .name = "snap-notes",     .activate = action_snap_notes },
    { .name = "snap-compare",   .activate = action_snap_compare },
};
